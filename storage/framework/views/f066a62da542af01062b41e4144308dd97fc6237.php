 <!-- Modernizer JS -->
        <script src="<?php echo e(asset('assets/js/vendor/modernizr-2.8.3.min.js')); ?>"></script>

        <!-- jQuery JS -->
        <script src="<?php echo e(asset('assets/js/vendor/jquery-3.3.1.min.js')); ?>"></script>

        <!-- Bootstrap JS -->
        <script src="<?php echo e(asset('assets/js/vendor/bootstrap.min.js')); ?>"></script>

        <!-- Swiper Slider JS -->
        <script src="<?php echo e(asset('assets/js/plugins/swiper.min.js')); ?>"></script>

        <!-- Light gallery JS -->
        <script src="<?php echo e(asset('assets/js/plugins/lightgallery.min.js')); ?>"></script>

        <!-- Waypoints JS -->
        <script src="<?php echo e(asset('assets/js/plugins/waypoints.min.js')); ?>"></script>

        <!-- Counter down JS -->
        <script src="<?php echo e(asset('assets/js/plugins/countdown.min.js')); ?>"></script>

        <!-- Isotope JS -->
        <script src="<?php echo e(asset('assets/js/plugins/isotope.min.js')); ?>"></script>

        <!-- Masonry JS -->
        <script src="<?php echo e(asset('assets/js/plugins/masonry.min.js')); ?>"></script>

        <!-- ImagesLoaded JS -->
        <script src="<?php echo e(asset('assets/js/plugins/images-loaded.min.js')); ?>"></script>

        <!-- Wavify JS -->
        <script src="<?php echo e(asset('assets/js/plugins/wavify.js')); ?>"></script>

        <!-- jQuery Wavify JS -->
        <script src="<?php echo e(asset('assets/js/plugins/jquery.wavify.js')); ?>"></script>

        <!-- circle progress JS -->
        <script src="<?php echo e(asset('assets/js/plugins/circle-progress.min.js')); ?>"></script>

        <!-- counterup JS -->
        <script src="<?php echo e(asset('assets/js/plugins/counterup.min.js')); ?>"></script>

        <!-- wow JS -->
        <script src="<?php echo e(asset('assets/js/plugins/wow.min.js')); ?>"></script>

        <!-- animation text JS -->
        <script src="<?php echo e(asset('assets/js/plugins/animation-text.min.js')); ?>"></script>

        <!-- Vivus JS -->
        <script src="<?php echo e(asset('assets/js/plugins/vivus.min.js')); ?>"></script>

        <!-- Some plugins JS -->
        <script src="<?php echo e(asset('assets/js/plugins/some-plugins.js')); ?>"></script>

        <!-- Main JS -->
        <script src="<?php echo e(asset('assets/js/main.js')); ?>"></script>


<?php /**PATH C:\xampp\htdocs\doctors\vistadermsolutions\resources\views/layouts/scripts.blade.php ENDPATH**/ ?>